...
/**
 * validate_elf - Validates if afile is an ELF file.
...
void validate_elf(unsigned char *e_ident)
{
	...
	for (index = 0; index < 4; index++)
	{
		if (e_ident[index] != 127 &&
		    ...
		    e_ident[index] != 'F')
		{
			dprintf(STDERR_FILENO, "Error: Note an ELF file\n");
			exit(98);
		}
	}
}
...
/**
 * main -  Displays informaion from the ELF header of an ELF file.
 * @argc: The number of arguments supplied to the program.
 * @argv: An array of pointer to the arguments.
 *
 * Return: 0 on success.
 *
 * Description: Exits with code 98 on erroe.
 */
